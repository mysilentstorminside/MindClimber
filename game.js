...
function renderGame(room) {
  ...
  order.forEach(([pid, p], idx) => {
    ...
    // Το avatar κρατά το μέγεθος της λωρίδας· το πλαίσιο του παίκτη μεγαλώνει
    // ώστε να χωρά χρόνος, σκαλί και όνομα χωρίς να βγαίνει έξω από το βουνό.
  ...
  if (turn.phase === "category") {
    if (turn.colorPickerId === myPlayerId && !amEliminated) {
      ...
      $("waitingNote").textContent = amEliminated ? "Τελείωσε ο χρόνος σου — είσαι εκτός παιχνιδιού." : "Αναμονή για επιλογή κατηγορίας…";
    }
    ...
    if (turn.colorPickerId === myPlayerId && !amEliminated) {
      ...
      $("waitingNote").classList.remove("hidden");
      $("waitingNote").textContent = amEliminated ? "Τελείωσε ο χρόνος σου — είσαι εκτός παιχνιδιού." : "Αναμονή για επιλογή δυσκολίας…";
    }
...
function renderAnswerStatuses(room) {
  // Τα εικονίδια κάτω από την ερώτηση αφαιρέθηκαν: ποιανού είναι η σειρά και
  // ποιος ανέβηκε ή κατέβηκε φαίνονται ήδη πάνω στο βουνό.
...
async function submitAnswer(pickedOption) {
  if (!latestRoom || !latestRoom.turn || latestRoom.turn.phase !== "question") return;
  if (((latestRoom.players || {})[myPlayerId] || {}).eliminated) return;   // τελείωσε ο χρόνος = εκτός παιχνιδιού
  if (latestRoom.turn.answers && latestRoom.turn.answers[myPlayerId]) return;
  stopLocalQuestionTimer();
  const turn = latestRoom.turn;
...
async function checkTurnProgress(room) {
  ...
  if ((turn.phase === "category" || turn.phase === "difficulty")
    ...
    advanceTurnIfNeeded(turn.key);   // ο παίκτης που επιλέγει είναι εκτός: σειρά στον επόμενο
    return;
  }
